...
function Simulator() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!tax.data || !goal.data || !snapshot) return; setBusy(true); setError(null); setMessage(null); const values = new FormData(event.currentTarget); const nextCandidate = { name: String(values.get("name")), course: String(values.get("course")) as CourseCode, idKind: String(values.get("idKind")) as IdKind, trainerBonusRole: String(values.get("trainerRole")) as TrainerBonusRole || null }; try { const response = await api.simulate({ candidateName: nextCandidate.name, course: nextCandidate.course, idKind: nextCandidate.idKind, trainerBonusRole: nextCandidate.trainerBonusRole, period: snapshot.period, targetTitle: goal.data.targetTitle, taxProfile: tax.data }); setCandidate(nextCandidate); setResults(response.results); } catch (reason) { setError(reason instanceof Error ? reason.message : "計算できませんでした"); } finally { setBusy(false); } };
