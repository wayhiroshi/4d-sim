...
export const api = {
  ...
  updateMemberIdentity: (id: string, displayName: string, idKind: Member["idKind"], masterMemberId: string | null) => request<{ id: string; displayName: string; idKind: Member["idKind"] }>(`/api/v1/members/${encodeURIComponent(id)}/identity`, { method: "PATCH", body: JSON.stringify({ displayName, idKind, masterMemberId }) }),
  ...
  createSimulationMember: (member: Pick<SimulationMember, "displayName" | "parentMemberId" | "course" | "period" | "trainerBonusRole" | "idKind"> & { introducerMemberId?: string }) => request<SimulationMember>("/api/v1/simulation-members", { method: "POST", body: JSON.stringify(member) }),
  ...
  updateSimulationMemberIdentity: (id: string, displayName: string, idKind: Member["idKind"], period: string) => request<{ id: string; displayName: string; idKind: Member["idKind"] }>(`/api/v1/simulation-members/${encodeURIComponent(id)}/identity`, { method: "PATCH", body: JSON.stringify({ displayName, idKind, period }) }),
