...
import type {
  ...
  ForecastScenario,
  Goal,
  GrowthStorySimulationRequest,
  GrowthStorySimulationResult,
  LeaderTeamSimulationRequest,
...
export const api = {
  ...
  createSimulationMember: (member: Pick<SimulationMember, "displayName" | "parentMemberId" | "course" | "period" | "trainerBonusRole" | "idKind"> & { introducerMemberId?: string; masterMemberId?: string | null }) => request<SimulationMember>("/api/v1/simulation-members", { method: "POST", body: JSON.stringify(member) }),
  createSimulationMembers: (members: BatchSimulationMemberInput[]) => request<{ members: SimulationMember[] }>("/api/v1/simulation-members/batch", { method: "POST", body: JSON.stringify({ members }) }),
  ...
  updateSimulationMemberIdentity: (id: string, displayName: string, idKind: Member["idKind"], masterMemberId: string | null, parentMemberId: string, period: string) => request<{ id: string; displayName: string; idKind: Member["idKind"] }>(`/api/v1/simulation-members/${encodeURIComponent(id)}/identity`, { method: "PATCH", body: JSON.stringify({ displayName, idKind, masterMemberId, parentMemberId, period }) }),
  deleteSimulationMember: (id: string, period: string) => request<{ deleted: number }>(`/api/v1/simulation-members/${encodeURIComponent(id)}?period=${encodeURIComponent(period)}`, { method: "DELETE" }),
  ...
  simulateBatch: (payload: BatchSimulationRequest) => request<{ result: BatchSimulationResult }>("/api/v1/simulations/batch", { method: "POST", body: JSON.stringify(payload) }),
  simulateLeaderTeam: (payload: LeaderTeamSimulationRequest) => request<{ result: LeaderTeamSimulationResult }>("/api/v1/simulations/leader-team", { method: "POST", body: JSON.stringify(payload) }),
  simulateGrowthStory: (payload: GrowthStorySimulationRequest) => request<{ result: GrowthStorySimulationResult }>("/api/v1/simulations/story", { method: "POST", body: JSON.stringify(payload) }),
