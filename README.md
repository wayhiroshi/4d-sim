...
フォーデイズ会員向けの、非公式・個人用試算ツールです。React SPA、Hono API、Cloudflare D1を1つのCloudflare Workerへ配備します。MVPではAIを使わず、版管理した公式ルール設定から決定論的にタイトル、5種類のボーナス、配置候補、タイトル不足条件、条件付き将来試算を算出します。

正式な資格・報酬はフォーデイズ公式明細を優先してください。本アプリは収入、健康効果、タイトル取得を保証しません。公式ロゴ、原本資料、実会員ID、口座情報、病歴・診療・服薬情報は保存しません。

## 実装範囲

- Dashboard: タイトル進捗、営業月p.v.、総ボーナス、概算手取、試算で確認する不足条件
- 組織: スマホ用階層リスト、PC用2カラムツリー、購入履歴
- 商品: 価格、p.v.、換算数、数量選択による自動計算
- 配置: 全候補を仮複製して再計算し、安定した規則で上位3案を表示
- 一括配置: 最大20人を1人ずつ全候補へ仮配置し、目標タイトル、未達条件、2名合算を含む報酬差、安定IDの順で逐次選択。配置手順を確認して試算組織へ一括反映
- 2名合算試算: 自分と選択したパートナーを個別計算し、各人の総ボーナス・概算振込額と2名合計を比較
- サブID試算: 本人のサブIDを通常最大5件まで仮配置し、各IDのボーナスを本人収入へ合算
- 将来試算: 3・6・12か月、保守・標準・挑戦の明示前提
- 参考機能: 全タイトルとP・Sトレーナー資格の条件表、現在値、未達条件、DR取得2パターン
- トレーナーボーナス: 公式確認済み資格と担当方法に応じて初回購入分を試算
- CSV: 会員、月次購入のテンプレート、全行プレビュー、一括反映
- PWA: 静的アセットだけを保存し、`/api/*`は常にネットワーク取得

人物・関係性・フォローの管理は「つながりカルテ」を正本とし、Navigatorでは扱いません。配置シミュレーターの入力は計算だけでは保存されず、「試算組織へ追加」を押した案だけを仮メンバーとして保存します。将来アプリ間連携を追加する場合も、試算に必要な最小限のデータだけを受け取ります。

対象外はABCカルテ、活動管理、AI文章生成、AIチャット、LINE、カレンダー、PDF、公式サイトの自動操作、一般公開SaaSです。
...
本人が保有するサブIDは、通常上限を5件としてマスターIDへ紐づけます。報酬試算では各サブID単位で資格・5種類のボーナスを計算してから本人分へ合算し、税・手数料等の概算控除は合算後に一度だけ適用します。本部承認による5件超の例外はMVPの新規入力対象外です。

配置シミュレーターの2名合算モードは、自分と選択した有効なマスターIDをそれぞれ独立して計算した後に合算します。概算振込額は2名とも現在の共通税・控除設定を使うため、実際の登録状況や控除条件が異なる場合は各人の総ボーナスを基準に確認してください。

一括配置は、1人を追加するたびに現在の仮組織から全配置候補を再計算する決定論的な逐次最適方式です。20人の全組合せを総当たりする方式ではないため絶対的な最適解は保証しませんが、採用した順序、配置先、各段階のタイトル・未達条件・報酬差をすべて表示します。

### ショッピングモール招待販売との共有

`/Users/yoshiaki/Documents/Fordays`の販売・コンテンツプロジェクトでも、p.v.、コース別ライン率、タイトル条件、商品価格差額を独自計算しません。このリポジトリの版管理されたルール設定と計算処理を正本として使います。

タイトルなし・Natural DNコラーゲン・年間注文計画の試算:

```sh
npm run forecast:shopping-mall -- \
  --course A \
  --title NONE \
  --orders 0,1,2,4,6,8,10,12,14,17,21,25
```

直接招待は1次ラインとして標準換算p.v.へライン率を適用し、製品ごとの販売ボーナス（通常価格と会員価格の差額）と合算します。端数は既存の報酬エンジンと同様に注文単位で四捨五入します。本人の当月定期購入がない場合、正式なボーナスは発生しません。
...
初期版で作成済みの`prospects`と`activities`テーブルは、データ損失を避けるため削除しません。現在の画面とAPIからは読み書きせず、将来の整理時にバックアップ確認後の別マイグレーションで扱います。
